...
baidu_cates = {
    ...
    101:{
        ...
        'cond' : PL(
            ...
            maindomain.eq('youku.com') & title.contains('—综艺—'),
            maindomain.eq('iqiyi.com') & stitle.contains('综艺'),
            domain.eq('zy.tudou.com') ,          # （主页）
            domain.eq('zy.kankan.com') & stitle.contains('综艺'),
        ),
    },
    ...
    103:{
        ...
        'parent' : 1,
        'cond' : PL(
            maindomain.eq('youku.com') & title.contains('—电影—'),
            maindomain.eq('tudou.com') & title.contains('电影'),
            maindomain.eq('iqiyi.com') & stitle.contains('电影'),
            maindomain.eq('kankan.com') & stitle.contains('电影'),
            maindomain.eq('juchang.com') & stitle.contains('电影'),
            maindomain.eq('qire123.com') & title.contains('电影'),
            domain.eq('video.sina.com.cn') & title.unmatch('(第[0-9]{1,3}|全)[集|季]'),
            domain.eq('www.m1905.com') & toppath.inside(['vod', 'mdb']),
            domain.eq('dianying.cntv.cn'),
            domain.eq('v.qq.com') & title.unmatch('(第[0-9]{1,3}|全)[集|季]'),
            domain.eq('movie.letv.com') | (maindomain.eq('letv.com') & title.unmatch('[0-9]{2,} - ')),
            domain.eq('tv.sohu.com') & title.unmatch('(第[0-9]{1,3}|全)[集|季]'),
            maindomain.eq('hunantv.com') & title.contains('电影库'),
        )
    },
    104:{
        'title' : '视频短片',
        'parent' : 1,
        'cond' : PL(
            domain.eq('v.youku.com') & title.contains('—在线播放—'),
        ),
    },
    105:{
        'title' : '网络电视',
...
i8cates = {
    106 : {
        ...
        'parent':1,
        'cond': PL(
            maindomain.eq('youku.com') & stitle.contains('电视剧'),
            maindomain.eq('tudou.com') & stitle.contains('电视剧'),
            maindomain.eq('iqiyi.com') & stitle.contains('电视剧'),
            maindomain.eq('kankan.com') & stitle.contains('电视剧'),
            maindomain.eq('juchang.com') & stitle.contains('电视剧'),
            maindomain.eq('qire123.com') & (title.contains('电视剧') | title.match('(第[0-9]{1,3}|全)[集|季]')),
            domain.eq('video.sina.com.cn') & title.match('(第[0-9]{1,3}|全)[集|季]'),
            domain.eq('www.m1905.com') & toppath.inside(['vod', 'mdb']),
            domain.eq('dianshiju.cntv.cn'),
            domain.eq('v.qq.com') & title.match('(第[0-9]{1,3}|全)[集|季]'),
            domain.eq('tv.letv.com') & title.match('[0-9]{2,} - '),
            domain.eq('tv.sohu.com') & title.match('(第[0-9]{1,3}|全)[集|季]'),
            maindomain.eq('hunantv.com') & title.contains('电视剧库'),
