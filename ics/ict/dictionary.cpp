...
*/		   nTemp=nFoundPos+1;//Check its previous position
   ...
   if(FindInModifyTable(nPos,sWordDel,nHandle,&pPre))
   {
	     ...
	     if(pPre!=NULL)
			 pCur=pPre->next;
         while(pCur!=NULL && strcasecmp(pCur->data.sWord,sWordDel)==0&&(pCur->data.nHandle==nHandle||nHandle<0))
		 {
			 pTemp=pCur;
...
*/	   nTemp=nFoundPos+1;//Check its previous position
   ...
   if(FindInModifyTable(nPos,sWordGet,-1,&pPre))
   {
	     ...
	     if(pPre!=NULL)
			 pCur=pPre->next;
         while(pCur!=NULL && strcasecmp(pCur->data.sWord,sWordGet)==0)
		 {
...
bool CDictionary::FindInModifyTable(int nInnerCode,char *sWord,int nHandle,PWORD_CHAIN *pFindRet)
{
   ...
   pPre=NULL;
   while(pCur!=NULL&&(strcasecmp(pCur->data.sWord,sWord)<0||(strcasecmp(pCur->data.sWord,sWord)==0&&pCur->data.nHandle<nHandle)))
   ...
   if(pFindRet)
     *pFindRet=pPre;
   if(pCur!=NULL && strcasecmp(pCur->data.sWord,sWord)==0&&(pCur->data.nHandle==nHandle||nHandle<0))
...
bool CDictionary::MergePOS(int nHandle)
{
	...
	{//delete the memory of word item array in the dictionary
		...
		for(j=0;j<m_IndexTable[i].nCount;j++)
		{
			nCompare=strcasecmp(sWordPrev,m_IndexTable[i].pWordItemHead[j].sWord);
	...
	for( i=0;i<CC_NUM;i++)//Operation in the modify table
	{
         ...
         while(pCur!=NULL)
		 {
			 if(strcasecmp(pCur->data.sWord,sWordPrev)>0)
			 {//The new word
