...
COutput::COutput(CResult* r_)
	: r(r_)
{}

bool COutput::operator()(PWORD_RESULT pItem, char *sResult,bool bFirstWordIgnore)
{
	int i=0;
	char sTempBuffer[WORD_MAXLENGTH],sPOS[3];
	sPOS[2]=0;
	...
	{
		//Get the POS string
		if(r->m_nOutputFormat!=0)//Not PKU format
			r->PKU2973POS(pItem[i].nHandle,sPOS);
		else//PKU format
		{
			sPOS[0]=pItem[i].nHandle/256;
			sPOS[1]=pItem[i].nHandle%256;
		}
		sPOS[r->m_nOperateType]=0;//Set the sPOS with operate type
		
		if(r->m_nOutputFormat==0)//PKU format
		{
			sprintf(sTempBuffer,"%s",pItem[i].sWord);
		    strcat(sResult,sTempBuffer);
			if(sPOS[0]!=0)//need POS 
		...
		else if(r->m_nOutputFormat==1)//973 format
		{
			sprintf(sTempBuffer,"%s\\",pItem[i].sWord);
		    strcat(sResult,sTempBuffer);
			if(sPOS[0]!=0)//need POS 
		...
		else if(r->m_nOutputFormat==2)//XML format
		{
			if(sPOS[0]!=0)//POS
			{
				sprintf(sTempBuffer,"<any type=\042%s\042>",sPOS);
...
{
	return ParagraphProcessing(sParagraph, strlen(sParagraph), sResult);
}

bool CResult::ParagraphProcessing(char *sParagraph, unsigned int pLen, char *sResult)
{
	static COutput callback(this);
	return ParagraphProcessing(sParagraph, pLen, sResult, callback);
}

template<typename Callback>
bool CResult::ParagraphProcessing(char *sParagraph, unsigned int pLen, char *sResult, Callback& callbak)
{
...
*/
		...
		{//Reach end of a sentence.Get a whole sentence
			...
			if(sSentence[0]!=0&&strcmp(sSentence,SENTENCE_BEGIN)!=0)
			{
				...
				callbak(m_pResult[0],sSentenceResult,bFirstIgnore);//Output to the imediate result
				//bFirstIgnore=true;
