...
COMMAND_WATCH_FIFO=""
COMMAND_NC=$(which nc.openbsd 2>/dev/null) || COMMAND_NC=${COMMAND_NC:-$(which nc 2>/dev/null)}
COMMAND_INSTALL_APP="install.js"
COMMAND_LOGLINE="loger.js"
RESERVED_APP_NAME=("system" "root" "initsys")
DEPEND_UTILS=("netstat" "start-stop-daemon" "iptables" "node" "${COMMAND_NC}")
MOUNT_DIRECTORIES=("/usr" "/lib" "/proc" "/var" "/bin")
APPS_CHROOT="${APPS_ROOT}"

# limits
#DEFAULT_ULIMITS="-u 1 -n 512 -v 131072 -m 131072 -i 1024 -f 0 -t 10 -q 128000 -x 128"
DEFAULT_ULIMITS="-n 2048"
...
app_run_command="$(which node) ${portalfile} | ${COMMAND_LOGLINE} ${appname} ${appstdout}"
